...
export const POST = async(req,res)=>{
    ...
    const {data,error} = await supabase.from('activeBoys').insert({id:id})

    if(error)
    {
        return NextResponse.json({msg:error},{status:500})
    }
    return NextResponse.json({msg:"Delivery Boy Activated"},{status:200})
}
...
export const DELETE = async(req,res)=>{
    ...
    const {data,error} = await supabase.from('activeBoys').delete().eq('id',id)

    if(error)
    {
        return NextResponse.json({msg:error},{status:500})
    }
    return NextResponse.json({msg:'Delivery Boy Deactivated'},{status:200})
}