...
export const POST = async(req,res)=>{
    ...
    const {data,error} = await supabase.from('Reviews').insert(pack)

    if(error)
    {
        return NextResponse.json({msg:error},{status:500})
    }
    return NextResponse.json({msg:'Review Created'},{status:200})
}
...
export const GET = async(req,res)=>{
    ...
    const {data,error} = await supabase.from('Reviews').select('*').eq('product',id)
...
export const DELETE = async(req,res)=>{
    ...
    return NextResponse.json({msg:'Review deleted'},{status:200})
}